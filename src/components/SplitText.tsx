  ...
  useGSAP(
    () => {
      ...
      const assignTargets = (self: InstanceType<typeof GSAPSplitText>) => {
        if (splitType.includes('chars') && self.chars?.length)  { targets = self.chars;  return }
        if (splitType.includes('words') && self.words?.length)  { targets = self.words;  return }
        if (splitType.includes('lines') && self.lines?.length)  { targets = self.lines;  return }
